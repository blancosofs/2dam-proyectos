...
public class JuguetesDAO {
	...
	//aqui te pillas el molde y le metes los datos
		public static ArrayList<Juguete> leerObjetoJuguete(Connection conexion) {
			ArrayList<Juguete> juguetesArrayList = new ArrayList<>();
			try {
				...
				while (resultado.next()) {
					...
					Juguete j = new Juguete(idJuguete,Nombre,Descripcion,Precio,Cantidad_stock);
					juguetesArrayList.add(j);
				}
			} catch (SQLException e) {
				e.printStackTrace();
			}
			return juguetesArrayList;
		}
	...
	public static void insertarJuguete(Connection conexion, String nombre, String descripcion,
			double precio, int cantidad_stock) {
		try {
			String consulta = "INSERT INTO juguete VALUES (DEFAULT,?,?,?,?)";
			PreparedStatement sentencia = conexion.prepareStatement(consulta);
			...
			if (lineas > 0) {
				System.out.println("---\n Nombre: " + nombre + " Descripcion: " + descripcion + " Precio: " + precio + " Cantidad de stock: " + cantidad_stock + "\n-----");
			} else {
	...
	public static int sacarId(Connection conexion) {
		int idJugueteUpdate = 0;
		try {
			String consulta = "select max(idJuguete) as idJuguete from juguete;"; //max, no count
			...
			while (resultado.next()) {
				idJugueteUpdate = resultado.getInt(1);
				System.out.println("Funciona");
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return idJugueteUpdate;
	}
	
	public static void updateTablaStock(Connection conexion, int idParaStock ,int cantidad_stock) {
		try {
			String consulta = "insert into stock values(1, 1, ?, ? )";
			PreparedStatement sentencia = conexion.prepareStatement(consulta);
			
			sentencia.setInt(1, idParaStock);
			sentencia.setInt(2, cantidad_stock);
		
			
			int lineas = sentencia.executeUpdate();

			if (lineas > 0) {
				System.out.println("[info] Update de stock");
				
			} else {
				System.err.println("[error] No se pudo configurar el stock");
			}
