...
public class VentaDAO {
	...
	public static void insertarVenta(Connection conexion, LocalDate fechaSQL, double monto, String tipoPago,
		...
		try {
			...
			// stock con update [por implementar]
	...
	public static Stock extraerObjetoStock_ParaStand(Connection conexion, int idJugueteB) {
		Stock stock = null;
		try {
			String consulta = "SELECT * from stock WHERE JUGUETE_idJuguete =?";
			PreparedStatement sentencia = conexion.prepareStatement(consulta);
			...
			while (resultado.next()) {
				int stock_idStand = resultado.getInt("STAND_idStand");
				int stock_idZona = resultado.getInt("STAND_ZONA_idzona");
				int idJuguete = resultado.getInt("JUGUETE_idJuguete");
				int cantidad = resultado.getInt("CANTIDAD");

				stock = new Stock(stock_idStand, stock_idZona, idJuguete, cantidad);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public static Venta extraerObjetoVenta(Connection conexion, int idVentaC) {
		Venta v = null;
		try {
			...
			while (resultado.next()) {
				...
				v = new Venta(idVenta, Fecha, Monto, tipo_pago, idEmpleado, stock_idStand, stock_idZona, idJuguete);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	// idCAMBIO, MOTIVO,Fecha ,
	// STOCK_STAND_idStand_Original, STOCK_STAND_ZONA_idzona_Original
	// ,STOCK_JUGUETE_idJuguete_Original , STOCK_STAND_idStand_Nuevo,
	// STOCK_STAND_ZONA_idzona_Nuevo, STOCK_JUGUETE_idJuguete_Nuevo,
	// EMPLEADO_idEMPLEADO
	public static void insertarCambio(Connection conexion, String motivo, LocalDate fechaSQL, int idStandCV,
			int idZonaCV, int idJugueteCV, int idStandC, int idZonaC, int idJugueteC, int idEmpleadoC) {
		try {

			String consultaInsert = "INSERT INTO cambio VALUES (DEFAULT,?,?,?,?,?,?,?,?,?);";
			PreparedStatement sentencia = conexion.prepareStatement(consultaInsert);

			sentencia.setString(1, motivo);
			sentencia.setDate(2, java.sql.Date.valueOf(fechaSQL));
			sentencia.setInt(3, idStandCV);
			sentencia.setInt(4, idZonaCV);
			sentencia.setInt(5, idJugueteCV);
			sentencia.setInt(6, idStandC);
			sentencia.setInt(7, idZonaC);
			sentencia.setInt(8, idJugueteC);
			sentencia.setInt(9, idEmpleadoC);
